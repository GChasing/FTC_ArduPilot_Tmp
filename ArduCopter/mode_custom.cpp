...
void ModeCustom::run()
{
    ...
    // gcs().send_text(MAV_SEVERITY_DEBUG, "u %5.3f", (double)velocity_NED[0]);

    Vector3f angular_velocity_Kb = ahrs_.get_gyro_latest();

    // gcs().send_text(MAV_SEVERITY_DEBUG, "mode 29 update function is running");
    ...
    // gcs().send_text(MAV_SEVERITY_CRITICAL,"%d", (int)motors->get_spool_state());
